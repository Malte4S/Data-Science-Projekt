...
def interactive_spearman_plotter(regional_daily, weather):
    while True:
        print("\n" + "="*60)
        print("  INTERACTIVE SPEARMAN CORRELATION GRAPHS")
        print("="*60)
        ...
        for w_var in available_w_vars:
            ...
            for r in regions:
                ...
                # We only attempt Spearman if we have at least 4 points (n-3 math requirement)
                if n_samples >= 4:
                    ...
                    label_text = (f"{r} (N={n_samples})\n"
                                  f"ρ = {rho:.2f} (p {p_val_str})\n"
                                  f"95% CI: [{ci_low:.2f}, {ci_high:.2f}]")
                    
                    plt.scatter(x, y, color=colors.get(r, 'gray'), alpha=dot_alpha, s=dot_size, 
                                edgecolors='black' if timeframe == 'Monthly' else 'none', label=label_text)
                else:
                    plt.scatter(x, y, color=colors.get(r, 'gray'), alpha=dot_alpha, s=dot_size, 
                                edgecolors='black' if timeframe == 'Monthly' else 'none',
                                label=f"{r} (N={n_samples}, Insufficient Data)")

            clean_var_name = w_var.replace('_', ' ').title()
            plt.title(f"Spearman Correlation: {timeframe} Capacity Factor vs. {clean_var_name} ({tech})", fontsize=14, fontweight='bold')
            plt.xlabel(f"Average {clean_var_name}", fontsize=12)
            plt.ylabel("Capacity Factor (CF)", fontsize=12)
            
            plt.legend(title="Region & Spearman Stats", loc='best', fontsize=9)
...
if __name__ == "__main__":
    ...
    interactive_spearman_plotter(reg_daily, w_data)
